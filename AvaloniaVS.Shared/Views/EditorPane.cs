    ...
    IVsTextEditorPropertyContainer
{
    ...
    private void InitializeEditorPane()
    {
        ...
        // dte.Mode / debugging state, see below
        SubscribeToDebugModeChanges();
    ...
    private sealed class BuildEventsSink(EditorPane owner) : IVsUpdateSolutionEvents2
    {
        private readonly EditorPane _owner = owner;

        public int UpdateSolution_Begin(ref int pfCancelUpdate)
        {
            _owner.HandleBuildBegin();
            return VSConstants.S_OK;
        }

        public int UpdateSolution_Done(int fSucceeded, int fModified, int fCancelCommand)
        {
            _owner.HandleBuildDone();
            return VSConstants.S_OK;
        }

        // Rest of IVsUpdateSolutionEvents2 members return S_OK / not implemented as no-ops
...
#pragma warning restore VSTHRD010
    ...
    private uint _debugContextCookie;
    private uint _selectionEventsCookie;

    private void SubscribeToDebugModeChanges()
    {
        ThreadHelper.ThrowIfNotOnUIThread();
        _monitorSelection = (IVsMonitorSelection)GetService(typeof(SVsShellMonitorSelection));
        var debugContext = VSConstants.UICONTEXT.Debugging_guid;
    ...
    private sealed class SelectionSink(EditorPane owner) : IVsSelectionEvents
    {
        private readonly EditorPane _owner = owner;
        ...
        public int OnSelectionChanged(IVsHierarchy h1, uint i1, IVsMultiItemSelect m1, ISelectionContainer c1, IVsHierarchy h2, uint i2, IVsMultiItemSelect m2, ISelectionContainer c2) => VSConstants.S_OK;
